...
[**rstan**](https://mc-stan.org/rstan/) is needed.
...
class, `nascSynth`, and a comparison helper for overlaying several
fitted models.
...
propagated to Step 2 (`n_samples` controls sample size). When
...
rescaled by the bias correction term, so that contamination of the donor
pool by spillovers from the treated unit is accountde for. If
`nasc_penalty = TRUE`, the log-likelihood includes a penalty term, which
discourages weight on units with strong network exposure to the treated
unit. When several *ρ* draws are propagated from Step 1, Step 2 is run
in parallel across them with `furrr` and the resulting posteriors are
pooled.
